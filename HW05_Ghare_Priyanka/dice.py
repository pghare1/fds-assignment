...
for i in range(sum_ub):
    odds['H'][i] = np.sum(priors[i+1:])/np.sum(priors)
    odds['E'][i] = priors[i]/np.sum(priors)
    odds['L'][i] = np.sum(priors[:i])/np.sum(priors)
...
with open(infilename, 'r') as f:
    ...
    for row in f.readlines():
        ...
        marginal_likelihood = np.sum(unnormalized_posteriors)
